...
var monitor = (function() {
	 ...
	 *	callback: error
	 */
	api.restartMonitor = function(monitor, callback) {
		api.stopMonitor();
	...
	api.startMonitor = function(monitor) {
		...
		if (!exe) {
			var exe = executor.runInSchedule({
				...
				//TODO get notifiers
				if(monitor.emails){
					api.sendEmail({url :monitor.url, notifiers: monitor.emails, value: value }, function(err, info){
						console.log("Email sent to " + monitor.emails + "\nurl:" + url);
					});
				}
				if(monitor.wechats){
					//TODO
